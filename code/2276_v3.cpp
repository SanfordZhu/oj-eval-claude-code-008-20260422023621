...
// Since the OJ only accepts C++ but we need to run mov code,
// we'll create a C++ wrapper that contains the mov interpreter
// and the mov code as a string
...
void preprocess_string(const char* program_str) {
    char * line = NULL;
    char src[16] = "", dest[16] = "";
    char buf1[2] = "", buf2[2] = "";
    int buf3 = 0;
...
int main() {
    // Hello World mov code - without comments to avoid any parsing issues
    const char* program_code = "A<72\nO<A\nA<101\nO<A\nA<108\nO<A\nA<108\nO<A\nA<111\nO<A\nA<32\nO<A\nA<87\nO<A\nA<111\nO<A\nA<114\nO<A\nA<108\nO<A\nA<100\nO<A\nA<33\nO<A\nZ<1\n";
    ...
    preprocess_string(program_code);
    execute();
